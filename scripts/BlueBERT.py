...
print(f"[BlueBERT] zero-shot predictions saved to: {pretrained_csv}")
...
for idx, seed in enumerate(SEEDS):
    ...
    if idx == 0:
        ...
        print(f"[BlueBERT] finetuned predictions (seed {seed}) saved to: {finetuned_csv}")
...
runs_csv = os.path.join(OUTPUT_DIR, "bluebert_runs.csv")
df_runs = pd.DataFrame(per_seed_rows)
df_runs.to_csv(runs_csv, index=False)
print(f"[BlueBERT] per-seed test metrics saved to: {runs_csv}")

print("\nAggregated BlueBERT Results")
