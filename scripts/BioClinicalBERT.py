...
bio_labels = annotated_df['bio_labels'].apply(lambda x: x.split()).tolist()
...
pretrained_csv = os.path.join(OUTPUT_DIR, "pretrained_extracted_entities_bioclinicalbert.csv")
...
pd.DataFrame(fine_tuned_results).to_csv(os.path.join(OUTPUT_DIR, "fine_tuned_entities_bioclinicalbert.csv"), index=False)
