...
model = AutoModelForTokenClassification.from_pretrained(
    ...
    label2id=label2id,
    id2label=id2label,
    hidden_dropout_prob=0.1,  # Dropout from Table 8
...
#TrainingArguments (MATCHING TABLE 8)
...
# Run inference on raw text files
...
# Save outputs
pd.DataFrame(results_pretrained).to_csv(os.path.join(OUTPUT_DIR, "pretrained_entity_extraction_results.csv"), index=False)
pd.DataFrame(results_finetuned).to_csv(os.path.join(OUTPUT_DIR, "finetuned_entity_extraction_results.csv"), index=False)

print("Pretrained and finetuned predictions saved.")
